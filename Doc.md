...
| **Hanime NSFW Provider (`HanimeClient`)** | `Content/NSFW/Providers/HanimeClient.py`, `Core/provider_factory.py`, `scraper.py` | Reverse-engineered Hanime API v11 cryptographic handshake with AES-256-GCM token encryption and WASM signature generation; full-text in-memory catalog search across 3,500+ titles with master HLS stream resolution (1080p, 720p, 480p, 360p). |
| **Audio Preference System (Dub / Sub)** | `Core/BaseClient.py`, `scraper.py`, `Content/Anime/Providers/` | Added `-a / --audio {sub,dub,dual,all}` option and `AudioPreference` config; automatically prioritizes English Dubbed streams across AnimeSuge, AniDB, KissKh, and Nyaa. |
| **SQLite History & Incomplete Resume Manager** | `Core/history_manager.py`, `Core/BaseDownloader.py`, `scraper.py` | Embedded SQLite ledger (`~/.config/media-scraper/history.db`) tracking downloads, segments, sizes, and errors; exposed via `--history`, `--incomplete`, `--clear-history`, and interactive Main Menu. |
