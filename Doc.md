...
|-------|---------------|---------------|
| **Multi-Line Non-Overlapping Parallel Progress Bars** | `Core/BaseDownloader.py` | Implemented dynamic multi-line ANSI terminal cursor management (`\033[NA` / `\r\033[K`) with synchronized registry (`_active_bars`) so concurrent parallel downloads render on dedicated separate terminal lines without overlapping or flickering. |
...
| **Numbered Resolution Selection & Input Validation** | `scraper.py`, `Content/Anime/Providers/AnimeSugeClient.py` | Added numbered resolution list cards (`[1] 1080P • Full HD [default]`, `[2] 720P`), supported selecting via index number or resolution text, added contextual error handling for invalid user entries, and aggregated multi-resolution streams across AnimeSuge servers. |
| **Instant Resolution Discovery & Streamlined Link Resolution** | `scraper.py`, `Content/Anime/Providers/AnimeSugeClient.py`, `Content/Anime/Providers/AniDbClient.py`, `Content/Anime/Providers/NyaaClient.py` | Decoupled resolution probing from whole-queue episode link resolution: interactive sessions probe available resolutions using only 1 sample episode in <1s (or bypass probing instantly when predefined via `-r`), eliminating the 30–60s freeze on large episode queues. Optimized AnimeSuge to stop server probing once primary master playlist is resolved. |
| **KissKh Provider v1.4 Modernization** | `Content/Anime/Providers/KissKhClient.py`, `Content/Movies/Providers/KissKhClient.py`, `Content/Series/Providers/KissKhClient.py` | Updated `KissKhClient` across Anime, Movies, and TV Series to output proper `(series_dir, episode_prefix)` tuples in `set_out_names`, format standardized `{Series Title} - S01 - E01 - {res}P.mkv` filenames, support dictionary episode range filtering, and wrap individual episode requests in error-containment blocks. |
| **AnimeSuge HTTP 429 Rate Limit Handling** | `Core/BaseClient.py`, `Core/commons.py`, `Content/Anime/Providers/AnimeSugeClient.py` | Added progressive backoff with `Retry-After` header extraction, silent suppression for optional server probes, and throttled batch resolution concurrency (max 3 workers) to eliminate 429 crashes. |
| **Single-Screen Clean Step-by-Step UX** | `Core/commons.py`, `scraper.py` | Added `clear_screen` and `render_step_header` to clear the terminal between interactive steps while keeping the top Hero Header and breadcrumbs persistent. |
| **Standardized Plex/Jellyfin Media Naming** | `Core/BaseClient.py`, `Core/BaseDownloader.py`, all clients | Implemented `format_media_filename` producing `{Series Title} - S{season:02d} - E{episode:02d}.mkv` across all Anime, TV Series, and Movies. |
| **Eliminated Redundant Episode Display Prompt** | `Core/BaseClient.py`, all provider clients | Completely bypassed `_get_episode_range_to_show` interactive prompts for long series, rendering a clean compact overview and direct single download prompt. |
| **Multi-Slot Concurrent In-Place Dashboard** | `Core/BaseDownloader.py`, `scraper.py` | Removed loose pre-download printing statements outside the slot manager and mapped in-place progress bars with clean episode descriptors (`S01-E01`, `S01-E02`). |
...
| **Hanime NSFW Provider (`HanimeClient`)** | `Content/NSFW/Providers/HanimeClient.py`, `Core/provider_factory.py`, `scraper.py` | Reverse-engineered Hanime API v11 cryptographic handshake with AES-256-GCM token encryption and WASM signature generation; full-text in-memory catalog search across 3,500+ titles with master HLS stream resolution (1080p, 720p, 480p, 360p). Default folder: `{Anime}/Hentai (NSFW)/{Series Name}`. |
| **Windows Optimizations & Python 3.12 Support** | `pyproject.toml`, `scraper.py`, `Core/commons.py`, `README.md` | Made `quickjs` dependency conditional (`< 3.13`) to eliminate MSVC C++ build errors on Windows/Python 3.13+, added automated Windows UTF-8 console stream reconfiguration (`sys.stdout.reconfigure`), VT100 / ANSI escape sequence initialization, and documented `winget install Gyan.FFmpeg` and execution policies. |
| **Interactive Audio Preference System (Sub / Dub)** | `Core/BaseClient.py`, `scraper.py`, `Content/Anime/Providers/` | Added interactive `AUDIO LANGUAGE PREFERENCE` menu (Sub preferred by default with instant Enter fallback) and `-a / --audio {sub,dub,dual,all}` option; automatically prioritizes English Dubbed streams and servers across AnimeSuge, AniDB, KissKh, and Nyaa. |
| **Selective Incomplete Task Resumption & Engine** | `Core/history_manager.py`, `scraper.py`, `Core/BaseDownloader.py` | Supported selective multi-task resumption (e.g. `1,3` or `1-4` or `all`), automatic link re-resolution for expired CDN tokens, and cache segment reuse across both the Task Manager (`[5] -> [2]`) and CLI (`--incomplete` / `-I`). |
| **HLS AES-128 Decryption Guard** | `Content/Anime/Downloaders/HLSDownloader.py` | Guarded fake header stripping exclusively to known image magic byte signatures (`PNG`, `JPEG`, `GIF`, `WEBP`) and bypassed stripping for encrypted ciphertext, preventing AES CBC-16 block boundary corruption and FFmpeg `non-existing PPS 0 referenced` errors. |
| **Dynamic Download Session Summary & Error Reporting** | `scraper.py`, `Core/commons.py` | Dynamic `[SUCCESS]`, `[PARTIAL]`, and `[FAILED]` summary receipts reflecting true download statuses with concise stderr error extraction. |
| **Hardware-Accelerated Video Compressor** | `Core/compressor.py`, `scraper.py`, `config_scraper.yaml` | Post-download video compression using auto-detected GPU encoders (`hevc_nvenc`, `hevc_vaapi`, `hevc_qsv`, `hevc_amf`, fallback `libx265`), achieving 50–70% disk savings with perceptual losslessness (CRF 23). |
| **Config Versioning & Automatic Migration** | `Core/commons.py`, `config_scraper.yaml` | Added `version: "1.5.0"` and `migrate_config()` auto-upgrading older configs on first launch while preserving user directory paths. |
